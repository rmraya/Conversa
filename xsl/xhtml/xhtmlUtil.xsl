...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:function name="u:flagsAttributeValue" as="xs:string">
    ...
    <xsl:variable name="props">
      <xsl:for-each select="($context/@ditac:flags-color, 
                             $context/@ditac:flags-background-color, 
                             $context/@ditac:flags-font-weight,
                             $context/@ditac:flags-font-style,
                             $context/@ditac:flags-text-decoration)">
        <xsl:sequence 
          select="concat(substring-after(local-name(),'flags-'),': ', ., ';')"/>
      </xsl:for-each>
    </xsl:variable>

    <xsl:sequence select="normalize-space(string-join($props, ' '))"/>
  </xsl:function>
  ...
  <xsl:template name="displayAttributes">
    ...
    <xsl:if test="$style ne ''">
      <!-- displayAttributes is invoked for elements which are fully flaggable.
           No need to concat the styles returned by u:flagsAttributeValue(). -->
