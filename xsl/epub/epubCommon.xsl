...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:function name="u:mediaType">
    ...
    <xsl:sequence
      select="if (ends-with($path2, '.gif')) then 'image/gif'
              ...
              else if (ends-with($path2, '.vtt')) then 'text/vtt'
              else if (ends-with($path2, '.srt')) then 'text/plain'
  ...
  <xsl:template name="appendObject">
    ...
    <xsl:for-each select="./*[contains(@class,' topic/param ')]">
      ...
      <xsl:if test="$src ne '' and
                    ($name eq 'source' or $name eq 'track' or 
                     $valuetype eq 'ref')">
        <xsl:call-template name="appendResource">
          <xsl:with-param name="href" select="$src"/>
          <!-- In the case of a media-track, @type is "subtitles", "captions",
               etc, and not the MIME type like for other params. Clearly a
               design error. -->
          <xsl:with-param name="mediaType"
            select="if ($name eq 'track') then '' else normalize-space(@type)"/>
        </xsl:call-template>
      </xsl:if>
    </xsl:for-each>
