...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="ditac:chunk">
    ...
    <xsl:if test="u:chunkIndex(base-uri()) eq 1">
      <xsl:if test="$coverImageName ne ''">
        <xsl:choose>
          <xsl:when test="$epub2-compatible eq 'yes'">
            <xsl:call-template name="generateCover">
              <!-- This template invokes copyCoverImage. -->
              <xsl:with-param name="basename" select="'__cover__.xhtml'"/>
            </xsl:call-template>
          </xsl:when>
          <xsl:otherwise>
      ...
      <xsl:if test="$epub2-compatible eq 'yes'">
  ...
  <xsl:template name="generateMetadata">
    <opf:metadata>
      ...
      <xsl:if test="$coverImageName ne '' and $epub2-compatible eq 'yes'">
  ...
  <xsl:template name="generateManifest">
    <opf:manifest>
      ...
      <xsl:if test="$epub2-compatible eq 'yes'">
        <opf:item id="__TOC_NCX" href="toc.ncx" 
      ...
      <xsl:if test="$coverImageName ne ''">
        <xsl:if test="$epub2-compatible eq 'yes'">
          <opf:item id="cover" href="__cover__.xhtml" 
                    media-type="application/xhtml+xml"/>
        </xsl:if>
        <!-- Otherwise, with EPUB3, an XHTML page containing the cover image
             is not useful. -->
      ...
      <xsl:for-each select="$ditacLists/ditac:chunkList/ditac:chunk">
        ...
        <xsl:variable name="properties">
          ...
          <xsl:if test="exists($titlePage//mml:math) or 
            ...
            <xsl:choose>
              <xsl:when test="$epub2-compatible eq 'yes'">
                <xsl:text> mathml switch</xsl:text>
              </xsl:when>
              <xsl:otherwise>
  ...
  <xsl:template name="generateSpine">
    <opf:spine>
      <xsl:if test="$epub2-compatible eq 'yes'">
        <xsl:attribute name="toc" select="'__TOC_NCX'"/>
      </xsl:if>
      <!-- But no need to reference __toc__.xhtml here. -->
